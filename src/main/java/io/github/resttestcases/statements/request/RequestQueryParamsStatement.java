...
public class RequestQueryParamsStatement extends LeafNodeStatement {
    ...
    public void config(NodePath path, NodeStatement parent, JsonNode node) {
        ...
        this.params = Utils.toMapKeyListTextValue(path, node);
    }

    @Override
    public void execute(Context context) {
        RequestSpecification req = context.getParam(RequestSpecification.class.getName());// Add in ParentNode
        for (Entry<String, List<String>> it : this.params.entrySet()) {
            req.queryParam(it.getKey(), it.getValue());
        }
    }
}
