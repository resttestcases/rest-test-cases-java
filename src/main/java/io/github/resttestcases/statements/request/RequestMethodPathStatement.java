...
public class RequestMethodPathStatement extends NodeValueStatement {

	@Override
	public void config(NodePath path, NodeStatement parent, JsonNode node) {
		super.config(path, parent, node);

		if (parent instanceof RequestStatement) {
			RequestStatement requestStatement = (RequestStatement) parent;
			requestStatement.httpMethodReq = Method.valueOf(path.getLast());
		}
	}
