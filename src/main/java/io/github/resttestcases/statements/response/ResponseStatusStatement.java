...
public class ResponseStatusStatement extends LeafNodeStatement {
	...
	public void valide(NodePath path, JsonNode node) {
		if (!node.isInt())
			throw new RestTestCasesException(
					"Invalid statement in " + path.toString() + ". Request status expects an integer number.");
	}
	...
	public void execute(Context context) {
		logger.debug("Valid response status code {}", status);
		ValidatableResponse response = context.getParam(ValidatableResponse.class.getName());
